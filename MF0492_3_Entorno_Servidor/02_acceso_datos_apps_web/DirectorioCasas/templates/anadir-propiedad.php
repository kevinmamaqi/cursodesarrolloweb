...
if ($existeValidacion && !$hayErrores) {
    echo "<div style='color: white; padding: 10px; border-radius: 10px; background-color: green; margin-bottom: 20px;'>Tu formulario se ha procesado adecuadamente.</div>";

    $hayError = $Validacion->guardarEnBD();

    if ($hayError) {
        echo "<div style='color: white; padding: 10px; border-radius: 10px; background-color: red; margin-bottom: 20px;'>No podemos establecer una conexión con la base de datos.</div>";
        var_dump($hayError);
    }
...
>
    ...
    <div class="grupo">
        ...
        <?php elseif ($existeValidacion && empty($Validacion->errores["n_pisos"])) : ?>
            <input class="valid-input" type="number" name="n_pisos" id="n_pisos" placeholder="Pisos" value="<?php echo $Validacion->n_pisos; ?>">
    ...
    <div class="grupo">
        <?php if ($existeValidacion && !empty($Validacion->errores["certificacion"])) : ?>
            <label style="color: red;" for="certificacion">Certificación energetica:</label>
            <?php if (!empty($Validacion->errores["certificacion"]["vacio"])) : ?>
                <?php echo $$Validacion->mensajes_error["certificacion"]["vacio"]; ?>
            <?php else : ?>
                <?php echo $$Validacion->mensajes_error["certificacion"]["erroneo"]; ?>
            <?php endif; ?>
        ...
        <select name="certificacion" id="certificacion">
            ...
            <?php foreach ($Validacion->certificaciones_validas as $key => $value) : ?>
                <option
                    value="<?php echo $key; ?>"
                    <?php echo $Validacion->certificacion === $key ? "selected" : ""; ?>
                >
                    <?php echo $value; ?>
                </option>
            <?php endforeach; ?>
    ...
    <div class="grupo">
        <?php if ($existeValidacion && !empty($Validacion->errores["usuario"])) : ?>
            <label style="color: red;" for="usuario">Selecciona usuario:</label>
            <?php if (!empty($Validacion->errores["usuario"]["vacio"])) : ?>
                <?php echo $Validacion->mensajes_error["usuario"]["vacio"]; ?>
            <?php else : ?>
                <?php echo $Validacion->mensajes_error["usuario"]["erroneo"]; ?>
            <?php endif; ?>
        <?php else : ?>
        ...
        <select name="usuario" id="usuario">
            ...
            <?php foreach ($Validacion->propietarios_validos as $key => $value) : ?>
                <option
                    value="<?php echo $key; ?>"
                    <?php echo intval($Validacion->usuario) === $key ? "selected" : ""; ?>
                >
                    <?php echo $value; ?>
                </option>
            <?php endforeach; ?>
